...
def partition(dataset, num_users, num_classes, alpha=0.1):
        min_size = 0
        min_require_size = 10
        K = num_classes
        labels = np.array(dataset.targets, dtype='int64')
        # labels = np.array(dataset.labels, dtype='int64') #svnh   #####**************************
...
def get_dataset(dataset, data_path, batch_size=64):
    if dataset == 'MNIST':
        channel = 1
        im_size = (28, 28)
        num_classes = 10
        ...
        std = [0.3530]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.FashionMNIST(data_path, train=True, download=True, transform=transform).cuda() # no augmentation
        dst_test = datasets.FashionMNIST(data_path, train=False, download=True, transform=transform).cuda()
        ...
        num_classes = 200
        root_dir = 'data/tiny-imagenet-200'  # update to the actual path as needed
        ...
        dst_train = datasets.ImageFolder(root=root_dir + '/train', transform=transform)
        dst_test = datasets.ImageFolder(root=root_dir + '/val/images', transform=transform)  
        class_names = dst_train.classes
        ...
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4789, 0.4723, 0.4305]  # CINIC-10 mean
        std = [0.2421, 0.2383, 0.2587]  # CINIC-10 std
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.ImageFolder(root='data/CINIC-10/train', transform=transform)  # training set
        dst_test = datasets.ImageFolder(root='data/CINIC-10/test', transform=transform)  # test set
        class_names = dst_train.classes

    else:
        exit('unknown dataset: %s'%dataset)

    trainloader_full = torch.utils.data.DataLoader(dst_train, batch_size=batch_size, shuffle=False, num_workers=0) # pin memory
    testloader_full = torch.utils.data.DataLoader(dst_test, batch_size=batch_size, shuffle=False, num_workers=0) # pin memory
