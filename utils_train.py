...
def epoch(mode, dataloader, net, optimizer, criterion, args, Rc_smooth, aug,  normalize='none', get_d_thetas=False, grad=None, it_eval=None, net_copy=None):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    if args.device.startswith('cuda'):
        criterion = criterion.cuda()
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        soft_labels = nn.functional.softmax(output / 2, dim=1)  # [batch_size, num_classes]
        n_class = Rc_smooth.shape[0]
        T = torch.zeros(n_class, device=args.device)
        # print(Rc_smooth.shape)
        for c in range(n_class):
            mask = lab == c  # [batch_size,]
            if mask.sum() > 0:
        ...
        Rc_normalized = Rc_normalized.to(args.device)
        ...
        loss = criterion(output, lab, weight=None) + 0.01*loss_lgkm
        # print(loss)
        # loss = criterion(output, lab)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, logits_vector, lr=None, verbose=True, fast=False, weight=None):
    ...
    criterion = cross_entropy_loss_cust(args) # softlabel
    ...
    epsilon = 1e-8
    Rc_smooth = logits_vector + epsilon  # [num_classes,]
    for ep in range(Epoch):
        loss_train, acc_train, _ = epoch('train', trainloader, net, optimizer, criterion, args, Rc_smooth, aug=(not args.no_aug), normalize=args.normalize_input, it_eval=it_eval, net_copy=net_copy) # no_aug mutes aug everywhere but gm
        lr_schedule(ep)

        ## test on full testset
        # if (ep != 0 and (ep+1) % (Epoch // 2) == 0) or (args.match_mode == '1vN' and ep % 10 == 0):
        if (ep != 0 and (ep+1) % Epoch == 0) or (args.match_mode == '1vN' and ep % 10 == 0):
            loss_test, acc_test, _ = epoch('test', testloader, net, optimizer, criterion, args, Rc_smooth, aug=False, normalize='none') # real data
            if verbose: logging.info('%s Evaluate_%02d: epoch = %04d train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, (ep+1), loss_train, acc_train, acc_test))
            if fast: break
...
def evaluate_fullset(it_eval, net, loader, args, verbose=True):
    ...
    if loader is None:
        return -99
    net = net.to(args.device)
    criterion = cross_entropy_loss_cust(args)
